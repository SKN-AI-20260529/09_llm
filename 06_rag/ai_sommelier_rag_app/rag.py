...
# .env 읽기 함수
def _load_project_environment() -> None:   # 함수명 앞에 _는 내부에서만 쓰겠다는 것. 프라이빗. 그래서 파일명에도 _를 붙였던 것임.
    """현재 폴더로부터 상위 폴더로 이동하면서 가장 가까운 .env를 읽어와 환경 변수로 등록"""
    ...
    if dotenv_path:
        load_dotenv(
            ...
            override=False, #같은이름의 환경변수 있으면 덮어쓰지 X.
                    )
# .env 읽기 함수 호출
...
# 상수 선언
# LLM 모델 이름
CHAT_MODEL_NAME = os.getenv("OPENAI_CHAT_MODEL", "gpt-5.6-luna").strip() or 'gpt-5.6-luna'

# 임베딩 모델명
EMBEDDING_MODEL_NAME = os.getenv("OPENAI_EMBEDDING_MODEL", "text-embedding-3-small").strip() or 'text-embedding-3-small'
...
def search_wines(dish_flavor: str) -> dict[str, str]:
    ...
    # 임베딩 모델 생성
    # -> 풍미 query를 Pinecone Vector Store에서 비교하기 위하여 임베딩 수행 시 사용
...
def recommend_wines(payload: dict[str, str]) -> Runnable:
    ...
    # dict -> PromptValue -> AIMessage -> str(한국어 와인 추천 글)
    return prompt | _create_chat_model() | StrOutputParser()
