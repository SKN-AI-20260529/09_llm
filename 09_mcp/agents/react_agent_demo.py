...
"""실행 중인 Weather MCP Tool로 사용될 LangGraph"""
...
async def build_agent():
    ...
    return create_agent(
        ...
        system_prompt=(
            "더하기 계산은 add_via_mcp를 사용하고, 현재 날씨 조회는 get_current_weather_via_mcp를 사용한다."
...
async def main() -> None:
    ...
    run_paid_agent = os.getenv("RUN_PAID_AGENT", "true").lower() == "true"
...
# LLM에게 MCP 호출하는 것을 도구로 등록해서 LLM이 사용자 질문에 맞춰 알아서 MCP를 호출할 수 있게 함
# (기존 -> 사용자가 MCP 이름을 명시해야 함)